...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110112827372.png#pic_center)
...
##### 1. 堆空间被分割为三块空间
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181012594.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181052736.png#pic_center)
使用 CMS，年老代对象就地释放。它们不会被来回移动。这个空间不会被压缩除非发生 FullGC。

##### 3. 年轻代收集
从 Eden 和 Survivor 区复制活跃对象到另一个 Survivor 区。所有达到他们的年龄阈值的对象会晋升到年老代。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181140529.png#pic_center)
##### 4. 年轻代回收之后
一次年轻代垃圾收集之后，Eden 区和其中一个 Survivor 区被清空。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181214124.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181308413.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181358378.png#pic_center)

**注意**：未被标记的对象等于死亡对象

##### 3. 年老代收集-清除之后
清除阶段之后，你可以看到大量内存被释放。你还可以注意到没有进行压缩操作。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201110181441434.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/2020111110162665.png#pic_center)

Java 虚拟机启动时选定区域大小。Java 虚拟机通常会指定 2000 个左右的大小相等、每个大小范围在 1 到 32M 的区域。

#### 2. G1 堆空间分配
实际上，这些区域被映射成 Eden、Survivor、年老代空间的逻辑表述形式。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111101710713.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111101927727.png#pic_center)
**注意**：区域没有必要像旧的收集器一样是保持连续的。
#### 4. G1 的年轻代收集
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102002748.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102120565.png#pic_center)
关于 G1 的年轻代回收做以下总结：
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102315450.png#pic_center)
#### 2. 并发标记阶段
如果发现空区域(“X”标示的)，在重新标记阶段它们会被马上清除掉。当然，决定活性的审计信息也在此时被计算。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102352635.png#pic_center)
#### 3. 重新标记阶段
空的区域被清除和回收掉。所有区域的活性在此时计算。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102434620.png#pic_center)
#### 4. 复制/清理阶段
G1 选择活性最低的区域，这些区域能够以最快的速度回收。然后这些区域会在年轻代垃圾回收过程中被回收。在日志中被指示为`[GC pause (mixed)]`。所以年轻代和年老代在同一时间被回收。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102523895.png#pic_center)
#### 5. 复制/清理阶段之后
被选择的区域已经被回收和压缩到图中显示的深蓝色区和深绿色区中。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20201111102558997.png#pic_center)
## 3.5 总结
