...
- `AbstractMap` 是实现了 `Map` 的抽象类，它实现了 `Map` 中的核心 API。其它 `Map` 的实现类可以通过继承 `AbstractMap` 来减少重复编码。
...
### 3.2. LinkedHashMap 原理

#### LinkedHashMap 数据结构

**`LinkedHashMap` 通过维护一对 `LinkedHashMap.Entry` 类型的头尾指针，以双链表形式，保存所有数据**。
...
- [Java-HashMap 工作原理及实现(opens new window)](https://yikun.github.io/2015/04/01/Java-HashMap工作原理及实现)

- [Map 综述（二）：彻头彻尾理解 LinkedHashMap(opens new window)](https://blog.csdn.net/justloveyou_/article/details/71713781)

- [Java 集合系列 09 之 Map 架构(opens new window)](http://www.cnblogs.com/skywang12345/p/3308931.html)

- [Java 集合系列 13 之 WeakHashMap 详细介绍(源码解析)和使用示例](http://www.cnblogs.com/skywang12345/p/3311092.html)

- [内容来自此处](https://dunwu.github.io/javacore/container/java-container-map.html#_6-1-map-%E7%AE%80%E4%BB%8B)
